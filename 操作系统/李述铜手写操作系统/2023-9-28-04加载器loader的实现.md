...
![image-20231020215210952](E:\a学习\笔记\img\image-20231020215210952.png)

![image-20231020215130304](E:\a学习\笔记\img\image-20231020215130304.png)

> 编译器默认使用四个段对数据进行存放, .text放置代码, .rodata字符串以及常量等, .data保存全局变量已经初始化的代码以及函数内部的初始化为static, .bss变量没有初始化的全局变量(或者初始化为0的变量), stack存储的是局部变量, 行同类型的值会进行合并

+ 可以通过脚本来控制存放的位置

需要创建一个配置文件, 这一个文件的结尾是.lds

![image-20231020221354878](E:\a学习\笔记\img\image-20231020221354878.png)

![image-20231020221421037](E:\a学习\笔记\img\image-20231020221421037.png)
...
> 在这里可以设置文件的位置, 以及各个数据段的数据的位置前后

## 加载内核映像文件

![image-20231020223202664](E:\a学习\笔记\img\image-20231020223202664.png)
...
![image-20231020224403287](E:\a学习\笔记\img\image-20231020224403287.png)

> 左边是给链接器使用的, 我们只需要关心右侧的数据

ELF格式, 有一个文件头, 相当于一个结构体

![image-20231020224452753](E:\a学习\笔记\img\image-20231020224452753.png)

头部里面保存有指针, 指向一个表, 表里面有需要加载的数据的位置等信息

![image-20231020224851539](E:\a学习\笔记\img\image-20231020224851539.png)

> 将数据加载到对应的指定的位置p_addr

![image-20231020225133496](E:\a学习\笔记\img\image-20231020225133496.png)
...
![image-20231020230307421](E:\a学习\笔记\img\image-20231020230307421.png)
...
//这一个是ELF的文件头部的数据结构
typedef struct {
    char e_ident[EI_NIDENT];	//一些标志服
    Elf32_Half e_type;			//文件的类型
    Elf32_Half e_machine;      	 //使用的机器的类型 
    Elf32_Word e_version;		//文件的版本号
    Elf32_Addr e_entry;			//虚拟地址, 实际程序需要跳转的位置
    Elf32_Off e_phoff;			//这一点是文件的第一段的数据表的位置
    Elf32_Off e_shoff;
    Elf32_Word e_flags;
    Elf32_Half e_ehsize;
...
![image-20231020232315988](E:\a学习\笔记\img\image-20231020232315988-16978153981601.png)
...
![image-20231020235804103](E:\a学习\笔记\img\image-20231020235804103.png)

![image-20231020235733770](E:\a学习\笔记\img\image-20231020235733770.png)
