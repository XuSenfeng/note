...
`UARTx_UCR1`, bit0使能位, bit14自动检测波特率, 1的时候使能, 

`UARTx_UCR2`: bit0软件复位, 0的时候复位, 1是复位成功, bit1: 接收使能, bit2: 发送使能, bit5: 设置数据位, 0表示7位数据位, 1表示8位数据位, bit6设置停止位, 0为1位, 1为两位, bit7: 奇偶校验位, 0偶校验, bit8:是否使能校验, 

`UARTx_UCR3`: bit2必须一直设置为1
