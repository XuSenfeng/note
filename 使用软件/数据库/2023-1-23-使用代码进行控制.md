...
在实际使用的时候插入的数据如果是一个SQL语句的一部分, 可能会导致实际输入的数据出错

```python
sql = "select * from users where name='{}' and passwd='{}'".format(user, pwd)
```
...
```sql
select * from users where name=''or 1=1 --' and passwd='123'
```

所以使用的时候不要直接使用字符串拼接, 使用execute进行拼接
