...
完整支持 ES6 的类语法，并新增了 public、private、protected、readonly 四种访问修饰符
...
```typescript
// 类装饰器：在类定义时执行， 实际是处理构造函数的修饰器
function sealed(constructor: Function) {
	Object.seal(constructor);    //密封构造函数本身（Calculator）
	// 每个函数天生自带一个 `.prototype` 属性，是一个对象，叫原型对象
	// `class Calculator` 在 JS 底层本质就是一个构造函数
...
function log(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]) {
        ...
        const result = original.apply(this, args); // 调用 add 方法
...
装饰器本质就是**函数**，用 `@函数名` 语法，去包装 / 修改类、方法、属性