...
USB 设备 = 整栋写字楼（唯一物理实体，有唯一通信地址）

配置 = 写字楼的一套整体运营方案（同一时间只能启用一套）

接口 = 方案里的一间独立办公室 / 专属部门（对应一个独立功能）

端点 = 办公室的专属收发窗口（数据传输的唯一出入口，分收发方向）

```bash
USB设备（顶层物理实体，设备级控制端点0）
└── 激活的USB配置（同一时间仅1个生效）
    └── USB接口（bInterfaceClass=0x03，HID类接口，驱动绑定单元）
        ├── HID类描述符（狭义HID描述符，接口的专属附属描述符）
        │   ├── 报告描述符（HID核心，定义数据格式，HID类描述符索引）
        │   └── 物理描述符（可选，极少用，描述人体工学相关参数）
        └── HID专属端点（中断IN/OUT，从属该接口，传输HID业务数据）
```
...
- **bMaxPacketSize** 长度1字节，表示端点 0 支持的数据包的最大字节数量。数据包的大小可以为 8 个字节、 16 个字节、 32 个字节或 64 个字节。(端点0是设备配置使用的端点, USB通过端点管理设备通信), 一般是64

![image-20240917102957281](https://picture-01-1316374204.cos.ap-beijing.myqcloud.com/image/202409171029369.png)

- **idVender** 长度2字节，厂家ID号，由USB协会分配。实际可以乱写
- **idProduct** 长度2字节，产品ID号，由厂家自己决定，比较自由。
- **bcdDevice** 长度2字节。设备版本号
- **iManufacturer、 iProduct 和 iSerialNumber** 长度均为1字节，都是字符串描述符索引。 字符串描述符包括有关制造商、产品和序列号等信息。如果存在字符串描述符，这些变量应该指向其索引位置。如果没有任何字符串，那么应该将零值填充到各个字段内。写0表示不使用, 实际是可有可无的
...
- **bConfigurationValue** 1字节，当前的配置的值，设置配置请求时会发送一个配置值，如果某个配置的bConfigurationValue和它相匹配，就表示该配置被激活。用于区分不同的配置, 这一个在实际设置的的时候需要是一个大于零的数字, 因为设置配置的时候0有特殊的用处
...
- **bNumEndpoints** 1字节，表示该接口的端点数（不包括0端点）。填写实际使用的端点的数量
...
端点是 HID 数据传输的最小单元，HID 描述符定义的数据格式，最终通过对应端点完成收发，二者强匹配

**HID 接口专属端点**：

1. **强制要求**：HID 规范规定，每个 HID 接口必须至少配备 1 个**中断 IN 端点**（方向：设备→主机），用于周期性向主机发送 HID 输入报告（如鼠标坐标、键盘按键码），这是 HID 业务数据的核心传输通道。
2. **可选配置**：可额外配备 1 个**中断 OUT 端点**（方向：主机→设备），用于主机向设备发送 HID 输出报告（如键盘指示灯状态）；若无该端点，输出报告可通过端点 0 的控制传输完成。
...
- **bEndpointAddress** 1字节，表示该端点的地址和方向。可以使1-15, 从设备发送到主机的时候bit7为1
- **bmAttributes** 1字节，表示该端点的属性，详细看上表。可以使用0x03, 主机定时请求轮训
- **wMaxPacketSize** 2字节，表示该端点的数据包最大长度。0x40 0x00发送64字节
- **bInterval** 1字节, 轮训时间0x0a
...
报告描述符的核心工作，就是定义这三类报告的完整格式

1. **输入报告（Input Report）**：方向为**设备→主机**，是最核心的报告类型。用于设备向主机上报状态，比如鼠标坐标、键盘按键码、游戏杆摇杆位置，通常通过 HID 接口专属的中断 IN 端点周期性传输，保证低延迟。
2. **输出报告（Output Report）**：方向为**主机→设备**。用于主机向设备下发控制指令，比如键盘大小写指示灯、设备震动反馈、显示屏刷新数据，可通过中断 OUT 端点传输，也可通过端点 0 的控制传输下发。
3. **特性报告（Feature Report）**：双向可读写，用于设备的参数配置与状态查询，比如鼠标 DPI 设置、宏键映射、设备固件版本读取，仅能通过端点 0 的控制传输进行读写。

+ 报表描述符定义了执行设备功能的数据格式和使用方法。
+ 报表描述符和 USB 的其他描述符是不一样的，它不是一个简单的表格
+ 报表描述符非常复杂而有弹性，因为它需要处理各种用途的设备报表的数据必须以简洁的格式来储存，这样才不会浪费
...
报告描述符本身是一段紧凑的二进制字节流，没有固定长度，由若干个**数据项**按顺序拼接而成，几乎所有场景都使用「短项」（长项为 USB 规范预留，极少使用）
...
主项是报告描述符的顶层结构，用于定义报告的类型、数据集合的边界，是整个描述符的骨架。主项的生效范围是**全局**的，直接决定了报告的核心结构
...
| 主项名称       | 编码 | 核心作用                                                     |
| -------------- | ---- | ------------------------------------------------------------ |
| Input          | 0x81 | 定义一个输入报告的数据字段，声明该字段的属性（如是否可变、绝对值 / 相对值） |
| Output         | 0x91 | 定义一个输出报告的数据字段，声明对应的属性                   |
| Feature        | 0xB1 | 定义一个特性报告的数据字段，声明对应的属性                   |
| Collection     | 0xA1 | 集合的起始，用于将一组相关的数据字段打包，必须与 End Collection 成对出现 |
| End Collection | 0xC0 | 集合的结束，闭合最近的一个 Collection                        |
...
一旦声明，**对后续所有主项持续生效**，直到被同类型的全局项覆盖, 这一个主要适用于定义一个用途页, 数据域的长度, 数量, 报告的ID
...
| 全局项名称      | 编码 | 核心作用                                                | 典型示例                                                     |
| --------------- | ---- | ------------------------------------------------------- | ------------------------------------------------------------ |
| Usage Page      | 0x05 | 用途页，定义后续数据的用途大类，是 HID 语义的基础       | 0x01（通用桌面页，鼠标 / 键盘 / 游戏杆）、0x07（键盘 / 按键页）、0x09（按钮页）、0x08（LED 页） |
| Logical Minimum | 0x15 | 逻辑最小值，定义数据字段的取值下限                      | 按键的 0（释放）、鼠标坐标的 - 127                           |
| Logical Maximum | 0x25 | 逻辑最大值，定义数据字段的取值上限                      | 按键的 1（按下）、鼠标坐标的 127                             |
| Report Size     | 0x75 | 单个数据字段的位宽（单位：bit）                         | 1bit 的按键、8bit 的坐标值                                   |
| Report Count    | 0x95 | 当前定义的同属性数据字段的数量                          | 3 个按键，对应 Report Count=3                                |
| Report ID       | 0x85 | 报告 ID，可选，用于给同一 HID 接口的多个报告分配唯一 ID | 报告 ID 1 对应键盘报告，ID 2 对应鼠标报告                    |

![img](https://pic3.zhimg.com/80/v2-33b971ae0acb079a3054b4be8cd622b8_720w.webp)

##### Local Item

**仅对紧随其后的第一个主项生效，主项执行完成后立即失效，不会影响后续的任何主项**, 定义数据的控制特性，如该数据域的用途，用途的最大值，用途的最小值等。
...
| 局部项名称    | 编码 | 核心作用                                                     | 典型示例                                        |
| ------------- | ---- | ------------------------------------------------------------ | ----------------------------------------------- |
| Usage         | 0x09 | 具体用途，在 Usage Page 的基础上，定义单个数据字段的具体功能 | 通用桌面页下，0x30=X 轴、0x31=Y 轴、0x02 = 鼠标 |
| Usage Minimum | 0x19 | 用途最小值，批量定义一组连续的用途起始值                     | 按钮页下，Usage Min=0x01，代表第一个按钮        |
| Usage Maximum | 0x29 | 用途最大值，批量定义一组连续的用途结束值                     | 按钮页下，Usage Max=0x03，代表第三个按钮        |
