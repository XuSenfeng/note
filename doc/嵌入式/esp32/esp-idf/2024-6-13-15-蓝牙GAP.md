...
> 基础知识[[2024-6-9-14-蓝牙]][[2024-6-13-16-蓝牙GATT]]
> 实际使用[[2024-6-13-17-SMP安全配对绑定]][[2024-6-22-18-蓝牙HID控制]]

GAP（**Generic Access Profile，通用访问配置文件**）是蓝牙协议栈中最基础、最重要的配置文件之一，定义了蓝牙设备如何发现、连接、安全交互及管理广播行为，是确保不同厂商设备互操作性的核心规范。可以把它看作蓝牙设备的 "身份证" 和 "外交官"，负责设备在蓝牙世界中的**可见性、连接建立和基本安全保障**

低功耗蓝牙（BLE）定义了**四种核心 GAP 角色**

| 角色                       | 类型     | 核心职责                                                     | 典型应用                       |
| -------------------------- | -------- | ------------------------------------------------------------ | ------------------------------ |
| **广播者（Broadcaster）**  | 无连接   | 仅发送广播数据，不接受连接请求                               | 蓝牙信标（Beacon）、环境传感器 |
| **观察者（Observer）**     | 无连接   | 仅扫描并接收广播数据，不发起连接                             | 信标接收器、环境监测设备       |
| **外围设备（Peripheral）** | 连接导向 | 广播可连接数据包，作为链路层从机（Slave），通常仅支持一个连接 | 智能手环、心率监测器、蓝牙键盘 |
| **中央设备（Central）**    | 连接导向 | 扫描并发起连接，作为链路层主机（Master），可同时连接多个外围设备 | 智能手机、平板电脑、智能手表   |

![image-20240624101046145](https://picture-01-1316374204.cos.ap-beijing.myqcloud.com/image/202406241010267.png)

| 核心功能     | 详细说明                                                     |
| ------------ | ------------------------------------------------------------ |
| **设备发现** | 定义广播（Advertising）和扫描（Scanning）机制，让设备彼此可见 |
| **连接建立** | 规范连接发起、响应和参数协商流程，支持连接导向和无连接两种通信模式 |
| **安全管理** | 提供配对（Pairing）、绑定（Bonding）和加密机制，保障数据传输安全 |
| **设备标识** | 定义设备名称、地址类型（公共地址 / 随机地址）和外观等标识信息 |
| **链路管理** | 处理连接参数更新、链路断开和角色切换等链路层操作             |
...
**蓝牙设备只发广播、不建立连接，像灯塔一样持续向外发射固定信息，让附近设备 “知道它在这里”。**
...
**所有 BLE 广播都必须遵守的空中包格式**，Eddystone/iBeacon/ 普通 BLE 设备都用这个壳
...
> + Header头
>
> 第一byte是一些类型, 使用bit6, bit7记录发送的信息是公开的(0)还是随机的(1)
>
> +   0000：ADV_IND（可连接通用广播）
> +   0010：**ADV_NONCONN_IND**（不可连接广播，Beacon 专用）
> +   0011：SCAN_REQ；0100：SCAN_RSP；0101：CONNECT_REQ
>
> 第二是一个长度, 这一个长度记录的是数据不包括CRC校验的长度
>
> + 数据
>
> 由一到多个 **AD Structure** 组成：
>
> ```
> 长度(1B) + 类型(1B) + 数据(N B)
> ```
>
> 最少是6字节, 6byte记录自己的地址, 之后是实际的数据, 这里的数据是很多AD结构构成的
...
> | 位            | 含义                                      | 说明                                                         |
> | ------------- | ----------------------------------------- | ------------------------------------------------------------ |
> | **bit0**      | 有限发现模式（Limited Discoverable Mode） | 置 1 时，设备只会**短时间内（通常 30 秒）对外广播**，适合临时配对场景（比如耳机首次配对），之后会自动隐藏以省电 |
> | **bit1**      | 普通发现模式（General Discoverable Mode） | 置 1 时，设备会**持续对外广播**，随时能被扫描到，比如一直可连接的智能手环、蓝牙键盘 |
> | **bit2**      | 不支持 BR/EDR（No BR/EDR）                | 置 1 时，表示设备**不支持经典蓝牙，仅支持 BLE**，比如 Beacon 信标、大部分物联网传感器都是纯 BLE 设备，会把这位置 1 |
> | **bit3**      | 同时支持 BLE 和 BR/EDR 控制器             | 置 1 时，表示设备的**蓝牙硬件控制器**同时支持 BLE 和经典蓝牙两种模式（双模硬件） |
> | **bit4**      | 同时支持 BLE 和 BR/EDR 主机               | 置 1 时，表示设备的 **软件协议栈(主机层)** 同时支持 BLE 和经典蓝牙的逻辑处理 |
> | **bit5~bit7** | 保留（Reserved）                          | 蓝牙规范预留位，目前无定义，必须设为 0                       |
...
作为蓝牙观察者（Observer），主动扫描周围所有 Eddystone 信标，自动解析 UID/URL/TLM 三种广播帧，并把信标的信息（地址、信号、ID、网址、电池温度）打印出来
...
static esp_ble_scan_params_t ble_scan_params = {
    ...
    .scan_duplicate         = BLE_SCAN_DUPLICATE_DISABLE  // 不过滤重复的包
};
...
ESP32 Eddystone UID 信标广播完整代码, 一个不带连接、只广播的 “电子身份牌 + 位置标签”

```c
// The length of adv data must be less than 31 bytes
//static uint8_t test_manufacturer[TEST_MANUFACTURER_DATA_LEN] =  {0x12, 0x23, 0x45, 0x56};
//adv data 广播的数据包结构体, 广播的内容
static esp_ble_adv_data_t adv_data = {
    .set_scan_rsp = false,  //无需扫描响应（Beacon不响应）
...
static const char *TAG = "EDDYSTONE_UID_BEACON";

// 1. Eddystone 16位UUID固定：0xFEAA
static uint8_t adv_service_uuid16[] = {0xAA, 0xFE};
...
static esp_ble_adv_data_t adv_data = {
    ...
    .service_uuid_len = 2,          // 修正：16位UUID长度=2
    .p_service_uuid = adv_service_uuid16, // Eddystone UUID
};
...
static esp_ble_adv_params_t adv_params = {
    .adv_int_min = 0x20,
    .adv_int_max = 0x40,
    .adv_type = ADV_TYPE_NONCONN_IND,  // 修正：不可连接广播（Beacon标准）
    .own_addr_type = BLE_ADDR_TYPE_PUBLIC,
    .channel_map = ADV_CHNL_ALL,
    .adv_filter_policy = ADV_FILTER_ALLOW_SCAN_ANY_CON_ANY,
};

// 4. GAP 回调函数（广播必须在回调中启动）
static void gap_ble_callback(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event) {
        // 广播数据配置完成 → 启动广播
        case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT: {
            esp_ble_gap_start_advertising(&adv_params);
            ESP_LOGI(TAG, "Eddystone UID 广播已启动");
            break;
        }
        default:
            break;
...
void app_main(void)
{
    ...
    // 定义UID帧结构体
    eddystone_uid_t uid;
    // 命名空间（10字节）、实例ID（6字节）、校准功率=10dBm
    uint8_t namespace_id[10] = "hello12345";
    uint8_t instance_id[6]  = "123456";
    
    // 组装Eddystone UID帧
    int len = Eddystone_Set_UID(&uid, 10, namespace_id, instance_id);
    if (len == -1) {
        ESP_LOGE(TAG, "UID帧组装失败");
        return;
    }
