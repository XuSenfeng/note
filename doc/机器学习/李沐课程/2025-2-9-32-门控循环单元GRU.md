...
这里没有$R~t~$的话就和之前的RNN一样, 这里的$R~t~$是一个0和1之间的数字, 可以对H进行一次处理

$R~t~$和$H~t−1~$ 的元素相乘可以减少以往状态的影响。每当重置门Rt中的项接近1时，我 们恢复一个如中的普通的循环神经网络。对于重置门$R~t~$中所有接近0的项，候选隐状态是以$X~t~$作为输 入的多层感知机的结果。因此，任何预先存在的隐状态都会被重置为默认值。
