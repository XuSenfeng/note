...
所有硬件（串口、网卡、GPIO、定时器）产生的中断 → 先交给这个 GIC 控制器 → 再通知 CPU 处理

初始化**中断控制器**、设置其他中断控制器节点的描述格式, 实际初始化的时间比较早, 所以不是在设备树初始化的时候进行匹配进行的 Documentation/devicetree/bindings/arm/gic.txt

```json
intc: interrupt-controller@a01000 {
		compatible = "arm,cortex-a7-gic"; // 使用的芯片的名字
		#interrupt-cells = <3>; // 其他人描述中断需要使用的格式
...
管理某一种具体中断, 可以在Documentation/devicetree/bindings/gpio/fsl-imx-gpio.txt里面描述, 以及Documentation/devicetree/bindings/gpio/gpio-mxs.txt
...
gpio5: gpio@20ac000 {
				...
				clocks = <&clks IMX6UL_CLK_GPIO5>; // 工作使用的时钟
				gpio-controller;
				#gpio-cells = <2>;
...
使用某一种具体中断, Documentation/devicetree/bindings/gpio/gpio-mxs.txt
...
- irq：要申请的中断号, 一般是在设备树里面可以获取
- handler：中断处理函数
- flags：中断标志, 中断放入特征

>   `IRQF_TRIGGER_RISING、IRQF_TRIGGER_FALLING、IRQF_TRIGGER_HIGH、IRQF_TRIGGER_LOW `等
>
>   若设置了IRQF_DISABLED，表明中断处理程序是快速处理程序，快速处理程序被调用时屏蔽所有中断，慢速处理程序则不会屏蔽其他设备的驱动
...
include/linux/interrupt.h里面记录有更多的中断
...
自动读取设备树：`interrupt-parent` + `interrupts`, 自动把 **GPIO5_1 + 上升沿** 转换成内核虚拟中断号, **不用你手动写死中断号**（Linux 驱动标准写法，硬件无关）
...
+ 返回值：中断号。这是一个虚拟的中断号, 实际不是GIC的中断号, 只用于处理这个引脚的中断
...
1.   按键按下 → GPIO5_1 引脚触发 
2.   GPIO 控制器硬件置位 ISR 寄存器
3.   GPIO 控制器向 GIC 发出 **硬件中断 74**
4.   CPU 响应 GIC 74，进入**内核官方 GPIO 驱动**
5.   官方驱动读 ISR → 发现是引脚1
6.   内核找到**虚拟中断号 **
7.   调用你注册的 request_irq 对应的 handler
...
多个设备共享一根硬件中断线的情况在实际的硬件系统中广泛存在, 多个设备在申请中断时，都应该使用`IRQF_SHARED`标志，而且一个设备以`IRQF_SHARED 申请某中断成功的前提是该中断未被申请，或该中断虽然被申请了，但是之前申请该中断的所有设备也都以`IRQF_SHARED`标志申请该中断

在中断到来时，会遍历执行共享此中断的所有中断处理程序，直到某一个函数返回IRQ_HANDLED

在中断处理程序顶半部中，应迅速地根据硬件寄存器中的信息比照传入的dev_id 参数判断是否是本设备的中断，若不是，应迅速返回`IRQ_NONE`
